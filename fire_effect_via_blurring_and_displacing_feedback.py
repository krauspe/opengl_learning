...
#
#
# Quite a few things that I deemed infeasible to implement back then are easily possible in todays GC's, among them: dynamic convolution patterns, interpolation,
# correct side-to-side wrapping (clamp vertically, but wrap horizontally), arbitrary "fire pixel size". The example doesn't demonstrate this though.
#
# It even should be possible to distort the fire eg. to flow around objects, by simply not drawing a simple square in the blur/displace step but
# something else. However, due to the exponential nature of feedback processes, one ends up with bizarrely distorted images frequently.
#
# It appeared necessary to use two textures, since you don't want to be overwriting the texture you're reading from. However, I'm not aware if
#  render-to-texture would overwrite a texture while you're rendering instead of dumping all changes from a frame buffer after rendering.
#  I don't even know if this is specified
#---------------------------------------------------------------------------------------------------------
